...
| **`js/bgdata.js`** | 同上，由 `tools/build-bg.mjs` 產生。要改背卡的名稱或補寶可夢清單是去改 `js/bgevents.js`，那一份會覆蓋骨架 |
...
| **`img/bg/`** | 17 張背卡的本地備援圖。背卡現在連上游，這批是上游掛掉時的最後防線，刪掉就沒有備援了 |
| **`img/extra/`** | 5 張上游沒有的裝扮圖。原始來源沒有 CORS，所以才收進來，刪掉分享圖就畫不出這幾隻 |
