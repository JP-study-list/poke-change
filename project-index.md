...
| `js/kbdata.js` | `KB_CATS` `KB_ENTRIES` `KB_COUNT` | 知識條目的 metadata（slug、標題、摘要、分類、更新日、來源）。**手動維護**，跟上面那幾個自動產生的不一樣。內文不在這裡，在 `kb/_src/<slug>.html`；格子牆只讀這個檔，畫一面牆不必載入任何內文。目前 6 則（亮晶晶首飾、亮晶晶好朋友、遠距離交換、必定變亮晶晶、交換的星星沙子、搜尋術語規則）。**`search` 分類的第一則是搜尋術語規則**，其餘都是 `trade`。空陣列時第四顆檢視鈕與頁尾那條連結都不畫、`kb/` 底下也不產出任何檔案 |
...
- **浮出來的面板只走 `ui.setPop()`**：`state.pop` 是 `null`／`"filter"`／`"settings"`，
  ...
  因為底下每一段處理完都會 return
- **右欄一次只顯示一種**：條目詳情、背卡詳情、選寶可夢、**複製搜尋字串**。
  第四種是 `state.copy`，按交換表那顆複製鈕時開，**開的時候還沒寫剪貼簿**，
  窗裡那顆鈕（`data-docopy`）按下去才寫。開任一種之前要把另外三種清掉
...
| 知識頁的費用矩陣與大數字 | `.cost` / `.stat-row`，**按則手寫進 `kb/_src/`**。矩陣只對「兩個維度交叉出一格一個數字」的資料成立，目前只有「交換的星星沙子」用。**章節速覽 `.kb-toc` 已經拿掉**（2026-09-18，使用者覺得突兀），要做得重寫 |
