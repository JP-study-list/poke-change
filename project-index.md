...
| `js/kbdata.js` | `KB_CATS` `KB_ENTRIES` `KB_COUNT` | 知識條目的 metadata（slug、標題、摘要、分類、更新日、來源）。**手動維護**，跟上面那幾個自動產生的不一樣。內文不在這裡，在 `kb/_src/<slug>.html`；格子牆只讀這個檔，畫一面牆不必載入任何內文。目前 5 則（亮晶晶首飾、亮晶晶好朋友、遠距離交換、必定變亮晶晶、交換的星星沙子）。空陣列時第四顆檢視鈕與頁尾那條連結都不畫、`kb/` 底下也不產出任何檔案 |
