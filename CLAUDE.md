...
- **設定裡三個顯示選項的圖示就是開關**（2026-09-14，使用者要求）。
  關著是線條加淡框的圓形徽章，開著填 `--gold`、圖示轉白。**沒有方框勾了**，
  留著的話一行裡兩個東西都在講同一件事。圖示在 `js/ui.js` 的 `DISP_ICONS`。
  **深色模式那一項的圖跟著狀態換**（太陽↔月亮），另外兩項不換——
  它們沒有這種公認的對照，硬換只會變成兩個都看不懂的圖。
