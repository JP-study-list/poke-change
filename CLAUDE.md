...
- **超極巨化的名單不受「上游有沒有圖」限制**，這正是做成旗標賺到的。
  Dittobase 標了 17 隻已實裝，上游只有 13 張圖（皮卡丘、喵喵、灰塵山與
  長毛巨魔四隻 404，2026-09-16 實測）。當條目的時候沒圖就畫不出來，
  只能收 13 隻；現在 18 個 id 全部勾得到，那四隻**勾了不換圖而已**，
  備援鏈退回本體那張，不會破圖。上游哪天補圖，重跑 `build-dex` 自動接上，
  不必改任何程式碼。
...
- 上游補上皮卡丘、喵喵、灰塵山與長毛巨魔的超極巨化圖之後重跑 `build-dex`，
  它們的 `gmaxIcon` 會自己長出來，不必改程式碼（現在勾得到但不換圖）
