...
- **圖鑑的右欄在桌機也是彈出**（2026-09-12，使用者要求）。常駐的話點一隻
  就把內容區推窄，格子牆從 14 欄掉成 12 欄整片重排，左右看起來不對稱。
  做法是 `body.rail-pop`：1200 以上那段查詢全部限定 `body:not(.rail-pop)`，
  帶著這個 class 就自然沿用預設的 fixed 彈出，不必把 sticky 一條條覆寫回去。
  class 在 `drawDetail()` 裡跟著 `state.view === "dex"` 切換。
  **背卡不適用**，它的右欄沒點卡片時擺清單摘要，一直都在。
...
- **斷點有兩個**：900px（導覽掉到貼底的 bar、欄數講死）與 1200px（右欄常駐）。
  ...
  1200 以上現在只有背卡是常駐，圖鑑走彈出，見上面 `rail-pop` 那條。
