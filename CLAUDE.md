...
- **符號照遊戲裡那顆畫，而且不能放在實心圓裡**（2026-09-16，使用者要求）。
  上游**沒有**收那顆符號（搜過 48319 個檔案，只有極巨粒子的道具罐
  `mp_pack.png` 與貼紙包佔位圖），所以是照使用者給的截圖手工畫成 SVG path，
  放在 `js/ui.js` 的 `MAX_MARK_PATH`。**canvas 吃同一個字串**
  （`new Path2D(d)` 直接收 SVG path data），兩邊不會走樣。
