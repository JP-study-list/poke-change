...
這裡收 1473 個條目，其中 303 個裝扮橫跨一百多個物種，
...
- **極巨化是交換條件，超極巨化是條目**（2026-09-16，使用者要求）。
  兩件事很容易混為一談，分界是**外觀有沒有變**：
  一般的極巨化在 GO 裡只是個體身上的一個能力，圖還是同一張，
  所以它是第五個勾選條件（`item.max`），跟異色／XXL／XXS／背卡並列；
  超極巨化那 13 隻上游給了自己的圖與異色圖，外觀真的不一樣，
  所以是條目（`d6.fGIGANTAMAX`），從 `isTempEvo` 放行。
  超級進化與原始回歸仍然整批排除，它們沒有自己的圖。
...
- **超極巨化的背卡跟著本體**（`backgrounds.js` 的 `cardsFor`，2026-09-16）。
  背卡清單來自 Dittobase，而它**不收任何特殊型態**：Max Battle 抓到的
  一律記成本體 id，因為抓到的當下本來就是本體外觀。但那一隻確實可以是
  超極巨化的個體，所以查不到時退回本體（顫弦蠑螈沒有本體，指到高調）。
  代價是沒有 Max Battle 的活動卡也會出現在下拉裡，使用者確認接受——
  要擋得先知道哪幾場活動有 Max Battle，上游沒有這份資料。
...
- **超極巨化的 CP 走 `STATS_SAME_AS`，一筆一筆指名**。game master 沒給那 13 個
  型態自己的基礎數值，而極巨化加的是血量倍率與招式、不動基礎數值，
  所以指到本體是對的。**不可以寫成「查不到就退回本體」**——那正是原始回歸
  會拿到蓋歐卡數字的那條路。顫弦蠑螈沒有本體，指到高調形態（`AMPED`）。

- **Dittobase 說實裝不等於上游有圖**。它標了 17 隻超極巨化已實裝，
  上游只有 13 張圖，皮卡丘、喵喵、灰塵山與積怨番長四隻 404（2026-09-16 實測）。
  照這個站一貫的規則以圖為準，那四隻不收成條目，但**本體仍然在可極巨化名單裡**，
  所以還是勾得到極巨化。上游哪天補圖，重跑 build-dex 就會自己長出來。

- **`item.max` 沒有升儲存版本號**。多一個布林欄位對舊資料是相容的，
  `cleanItem` 讀到舊紀錄補 `false`；升 `v` 反而會讓舊版整包讀不到。
  代價是新版存的 `max` 給舊版讀會被洗掉，跟 v1 到 v2 一樣單向，
  但只丟一個旗標。check 有一條驗「舊紀錄沒有 max 欄位時補 false」。
...
| 圖鑑條目 | 1473 |
| 　一般 | 893 |
| 　型態變化（含地區型 53、超極巨化 13） | 277 |
| 　裝扮 | 303 |
| 已實裝異色 | 1423 |
| 可極巨化的條目 | 148 |
| 收納夾 / 背卡 / 收集格 | 23 / 240 / 1579 |
| 介面文字 | 三語各 110 個 key |
| 有 IV100 CP 的條目 | 1473（全部） |
