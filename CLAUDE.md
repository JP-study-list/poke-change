...
- **極巨化與超極巨化都是交換條件，不是條目**（2026-09-16，使用者要求）。
  ...
  **外觀沒有因此丟掉**：那 13 種的圖掛到本體的 `gmaxIcon`／`gmaxShinyIcon`，
  勾起來就換圖，跟異色同一個機制（`iconAttrs(e, shiny, gmax)`）。
  所以「不佔格子」與「看得出外觀」兩件事同時成立，不必二選一。

  超級進化、無極巨化與原始回歸仍然整批排除：它們連掛圖的位置都不需要，
...
- **極巨化的徽章在右上角，刪除鈕也在**。兩顆圓會差 1.5px 重疊，
  所以 hover 與編輯模式時徽章 `opacity: 0` 讓位——那一角一次只給一個圓。
  徽章不放進格子下方那行尺寸文字：「極巨化」三個字比 XXL 長太多，
  手機 68px 的格子排不下。
